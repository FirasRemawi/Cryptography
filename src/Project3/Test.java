...
public class Test extends AESComponents {
	...
	public String encrypt(String plaintext, byte[] key, String mode) throws AESException {
		...
		if (mode.equalsIgnoreCase("ECB")) {
			...
			for (int i = 0; i < paddedPlaintextBytes.length; i += 16) {
				byte[] block = Arrays.copyOfRange(paddedPlaintextBytes, i, i + 16);
				byte[] encryptedIv = encryptBlock(previousBlock, roundKeys); // Encrypt the IV or previous ciphertext
																				// block
				byte[] encryptedBlock = xorBlocks(block, encryptedIv); // XOR with plaintext block to get the ciphertext
																		// block
				previousBlock = encryptedBlock; // Use ciphertext block as the next IV
				cipherTextBuilder.append(Base64.getEncoder().encodeToString(encryptedBlock));
				cipherTextBuilder.append("\n"); // Append a newline for readability
			}
		} else {
		...
		return cipherTextBuilder.toString().trim(); // Ensure no extra whitespace
	}
	...
	public String decrypt(String base64CipherText, byte[] key, String mode) throws AESException {
		...
		if (mode.equalsIgnoreCase("ECB")) {
			...
			if (cipherTextBlocks.length < 2) {
				throw new AESException("Cipher text too short for CFB mode");
			}
			...
			for (int i = 1; i < cipherTextBlocks.length; i++) {
				try {
					byte[] cipherTextBytes = Base64.getDecoder().decode(cipherTextBlocks[i].trim()); // Clean and decode
					byte[] encryptedIv = encryptBlock(previousBlock, roundKeys); // Encrypt the IV or previous
																					// ciphertext block
					byte[] decryptedBlock = xorBlocks(cipherTextBytes, encryptedIv); // XOR with ciphertext block to get
																						// the plaintext block
					previousBlock = cipherTextBytes; // Use ciphertext block as the next IV
					plainTextBuilder.append(new String(decryptedBlock, StandardCharsets.UTF_8));
				} catch (IllegalArgumentException e) {
					throw new AESException("Invalid Base64 input: " + cipherTextBlocks[i], e);
				}
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			String encryptedASCII = aes.encrypt(plainText, generatedKey, "ecb");
